...
use self::serialize::base64::{Config,ToBase64,Standard};
...
pub fn hex_chars_to_values_base64(hex_string: &[u8]) -> String {
    hex_util::hex_chars_to_values(hex_string).to_base64(Config { char_set: Standard, pad: true, line_length: None})
}
    ...
    range(0x0u8, 0xFFu8).fold(initial_state, |current_state, next_cipher_key| {
        ...
        match String::from_utf8(decoded_vec) {
            Ok(new_decode) => {
                ...
                match english_text::character_score(new_decode[]) {
...
// Challenge 5
pub fn encode_line_with_repeating_key(text_string: &str, cipher_key: &str) -> String {
    ...
    let value_vec = hex_util::values_to_hex_chars(encoded_vec[]);
