...
pub fn detect_oracle_block_size<'a>(append_str: &'a str, try_up_to: usize) -> Result<usize, MatasanoError> {
    ...
    analyzer::detect_oracle_block_size(&mut |block| oracle.randomly_append_and_encrypt_text(block), try_up_to)
}
...
pub fn decrypt_append_str<'a>(append_str: &'a str) -> Result<String, MatasanoError> {
    ...
    let decoded_vec = decryptor::break_oracle_append_fn(&mut |block| oracle.randomly_append_and_encrypt_text(block))?;
